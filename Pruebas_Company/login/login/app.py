...
@app.route('/')
def show_all():
   return render_template('index.html', company=company.query.all())#company = engine.query.all() )
...
def show_login_form():
    ...
    if form.validate_on_submit():
        ...
        print("cur")
        cur.execute("SELECT nombreEmpresa FROM empresas WHERE cif LIKE '"+ cif + "' AND hashPass LIKE '" + password + "'")
        ...
        if (nombre is not None): 
            ...
            return render_template("espacio_empresa.html", nombre=nombre)
...
def show_signup_form():
    ...
    csrf.generate_csrf()
    if form.validate_on_submit():
        print("No he entrado en el validator")
        ...
        mysql.connection.commit()
        print("ha hecho el select")
        return render_template("espacio_empresa.html", nombre=nombreEmpresa)
